...
namespace DAL
{
    public class orderDAL
    {
        ...
        public List<orderdetail> GetAllItemFromLastOrderDetail(int? id)
        {
            ...
            reader.Close();
            command.CommandText = "select item_name,price,amount from orderdetail where order_id = " + id + ";";
            return GetAllItemFromOrderDetail(command);
        }
        private List<orderdetail> GetAllItemFromOrderDetail(MySqlCommand command)
        {
            MySqlConnection connection = DbConfiguration.OpenConnection();
            MySqlDataReader reader = null;
            ...
            while (reader.Read())
            {
                list.Add(GetItemFromOrderDetail(reader));
            }
            reader.Close();
            connection.Close();
            return list;
        }
        private orderdetail GetItemFromOrderDetail(MySqlDataReader reader)
        {
